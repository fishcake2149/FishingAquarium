...
world = r'''const pond={cx:70*TILE,cy:54*TILE,rx:23*TILE,ry:13*TILE};
...
'''
s, n = re.subn(r"const pond=\{.*?\n\nconst decoRand=", world + "\nconst decoRand=", s, flags=re.S)
assert n == 1, f'world block {n}'

collision = r'''function playerTouchesPond(x,y,r=18){const d=r*.72;return [[0,0],[r,0],[-r,0],[0,r],[0,-r],[d,d],[d,-d],[-d,d],[-d,-d]].some(([ox,oy])=>pondNorm(x+ox,y+oy)<1.018)}
...
s, n = re.subn(r"function playerTouchesPond\(.*?\nfunction isSolidVillage\(.*?\n", collision + "\n", s, flags=re.S)
assert n == 1, f'collision block {n}'
s = s.replace(
    "const x=(tx+.5)*TILE,y=(ty+.5)*TILE;if(pondNorm(x,y)<1.03)return true;",
    "const x=(tx+.5)*TILE,y=(ty+.5)*TILE;if(!isVillageLandPoint(x,y))return true;if(pondNorm(x,y)<1.03)return true;"
)

npc_defs = r'''const npcDefs=[
...
function pondScreenPath'''
s, n = re.subn(r"function traceWorldPoly.*?\nfunction pondScreenPath", terrain, s, flags=re.S)
assert n == 1, f'terrain {n}'
...
function drawTree'''
s, n = re.subn(r"function drawBuilding\(b\)\{.*?\nfunction drawTree", building, s, flags=re.S)
assert n == 1, f'building {n}'

p.write_text(s, encoding='utf-8')

ip = Path('prototype/v3/index.html')
h = ip.read_text(encoding='utf-8')
h = re.sub(r"game-v31-source\.js\?v=[^\"']+", 'game-v31-source.js?v=20260915-4', h)
